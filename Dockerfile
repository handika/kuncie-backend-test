...
RUN go build -ldflags "-s -w" -o /build/kuncie-be-test.app server.go
...
COPY --from=builder --chown=appuser:appuser /build/kuncie-be-test.app /app/
...
CMD ["sh", "-c", "/app/kuncie-be-test.app"]